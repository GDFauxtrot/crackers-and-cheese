        ...
        if (gameManager.gameIsRunning) {
            ...
            if (info.Key != -1) {
                if (movementType == PlatformMovementType.Normal || movementType == PlatformMovementType.NormalInvertedActivation) {
                    transform.position = Vector3.Lerp(startPoint, endPoint, (Mathf.Sin(info.Value * speed)+1)/2f);
                } else if (movementType == PlatformMovementType.StartToEnd) {
                    transform.position = Vector3.Lerp(startPoint, endPoint, info.Value);
                }
            } else {
                if (!warned) {
                    Debug.LogError("Moving platform at " + gameObject.transform.position + " was not picked up by the network manager for synchronization!");
                    warned = true;
                }
